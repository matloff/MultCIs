...
model-selection context. (General application of MI methods will be
presented as well.) 
...
using the latter in any statistical analysis.

## Motivating Example {#sec-motex}
...
This is US census data. Let's predict gender, using logistic regression,
random forests, XGBoost, and k-NN with 2 values of k..
...
considered a sample from some population, actual or conceptual. In the
ML community, the term *data-generating process* is analogous. Also note
the "hat" notation, '^' meaning "estimate of."]{.column-margin} To set
the stage, let's review the statistical concepts of *confidence
interval* and *standard error*. Say we have an estimator
$\widehat{\theta}$ of some population parameter $\theta$, e.g.\
$\bar{X}$ for a population mean $\mu$.

Loosely speaking, the term *standard error* of $\widehat{\theta}$ is our
estimate of $\sqrt{Var(\widehat{\theta})}$.  More precisely,
suppose that $\widehat{\theta}$ is asymptotically normal/Gaussian.  The
standard error is an estimate of the standard deviation of that normal
distribution. For this reason, one sometimes writes
$AVar(\widehat{\theta})$ rather than $Var(\widehat{\theta})$. 

In the familiar case in which $\theta$ is a
population mean and $\widehat{\theta}$ is the sample mean, 

$$
s.e.(\widehat{\theta}) = \frac{s}{{\sqrt{n}}}
$${#eq-s2n}

where $s^2$ is the sample variance.  and where we denote the standard
error of $\widehat{\theta}$ by $\textrm{s.e.}(\widehat{\theta})$.
...
instance]{.column-margin}An approximate 95% confidence interval (CI)
for $\theta$ is then 

$$
\widehat{\theta} \pm 1.96 ~
\textrm{s.e.}(\widehat{\theta}) 
$$
...
$$
\widehat{\theta} \pm z_{\alpha} \textrm{s.e.}(\widehat{\theta})
$${#eq-pmse}
...
[The **qeLogit** function is a wrapper for **glm**. In order to
accommodate more than two classes, it performs multiple **glm** runs
storing their outputs in an R list **glmOuts**.]{.column-margin}
...
If in @eq-probAorB one replaces $B$ by $B_1 \textrm{ or } B_2$, that
extends the relation from two events to three, and so on.

For events $A_1,...,A_m$,

$$
P(A_1 \textrm{ or } ... \textrm{ or } A_m) \leq \sum_{i=1}^m P(A_i)
$$
...
* The *covariance matrix* of a random vector $X$ has as its row $i$,
  ...
  (elements denoted here by superscipts):
  ...
  where ' denotes matrix transpose and vectors are column vectors by
...
In R, say we have a data frame or matrix **A**, one data point per row.
Then **cov(A)** computes @eq-covformula.

Note too that in analogy with @eq-s2n, 
...
functions have this for their output, including say **glm**. When we
make the call **vcov(glmOut)**, the R interpreter sees that **glmOut**
is of class "glm" and thus transfers the call to the class-specific
function, **vcov.glm(glmOut)**.  The function **coef**, also used here,
is also generic.]{.column-margin} Parametric statistical models, e.g.
linear and logistic regression, parametric analysis of contingency
tables and so on, are smooth functions of sums, and thus asymptotically
normally distributed. Their covariance matrices arise as part of the
derivation, and are accessible for many R estimators via the **vcov**
function, such as in our logit model above:

```{r}
vcov(logitOut$glmOuts[[1]])
```

Keep in mind that our $\theta$ here is $\beta$, the vector of regression
coefficients. So, the covariance matrix printed out here is the
estimated asymptotic covariance matrix of $\widehat{\beta}$. So for
instance the estimated covariance between $\widehat{\beta}_{occ101}$ and
$\widehat{\beta}_{0}$ is about -0.00125.

### Multivariate normal distribution family

This is an extension of the ordinary normal family, with parameters
mean vector and covariance matrix. Again, "there is no such animal" in
practice, but fortunately there is a multivariate version of the Central
Limit Theorem, which makes it all work.
...
Say the random vector $X$ of length $p$ is multivariate normal, with
mean vector $\nu$ and (invertible) covariance matrix $\Lambda$. Then the
quantity
...
If $\nu$ and $\Lambda$ are estimated from the sample as above, the
distribution is approximate.

## Scheffe' CIs

> Say $\widehat{\theta}$ is an asymptotically $k$-dimensional normal
> estimator of some population value $\theta$. Denote the covariance
> matrix in that asymptotic distribution by $\Sigma$, assumed invertible,
> with estimate $\widehat{\Sigma}$. Denote the sample size by $n$.  Then
> for any constant, compatible vector $a$, form the interval
> 
> $$
> a' \widehat{\theta} \pm \sqrt{d_{\alpha} a' \widehat{\Sigma} a}
> = a' \widehat{\theta} \pm  \sqrt{d_{\alpha}} ~ s.e.(a' \widehat{\theta})
> $${#eq-scheffe}
> 
> where $d_{\alpha}$ is the upper $\alpha$ quantile of the chi-square
> distribution with $k$ degrees of freedom. 
> 
> Then the intervals in @eq-scheffe hold simultaneously at the approximate
> $(1-\alpha)$ confidence level.

[In the case $a = (0,0,0,0,1,-1,0,...,0)$, the sum of the elements 
of $a$ is 0, known as a *contrast*. If one is willing to restrict one's
analysis to contrasts, the degrees of freedom will decline by 1, making
for slightly narrower CIs.]{.column-margin}
The values of $a$ are chosen according to the analyst's interests. 
For instance, in our logit model above, we could take $a = (0,0,0,0,1,0,...,0)$
if we want a CI for $\beta_{occ101}$, or $a = (0,0,0,0,1,-1,0,...,0)$ if
we want a CI for the difference $\beta_{occ101} - \beta_{occ102}$.

## Example

Let's continue the example in @sec-motex.
...
t(a) %*% xbar  
# -0.01258
d * sqrt(t(a) %*% covxbar %*% a)
t(a) %*% xbar - d * sqrt(t(a) %*% covxbar %*% a)
t(a) %*% xbar + d * sqrt(t(a) %*% covxbar %*% a)
# CI is (-0.02174126,-0.003418737)
```

Again, note that we can form as many CIs this way as we want, yet still
retain an overall confidence level of at least 95%.

## Impact of the Sampling Scheme

Again consider the code in @sec-motex.  Since the random number seed is
not reset, each of the calls is using different training sets and
different test sets from each other. This makes them statistically
independent. The alternative would be to insert, say
...
* In the second scenario, the fact that each call operates on the same
  dataset
...
* There is a bit of drama in this word *contain* in the statement "95%
  of the resulting confidence intervals will contain $\theta$." Instead
  of saying the intervals *contain* $\theta$, why not simply say
  $\theta$ is *in* the intervals? Aren't these two descriptions
  equivalent in terms of English?  

  Of course they are.  But many instructors of statistics classes worry
  that students will take the description based on "in" to mean that
  $\theta$ is the random quantity, when in fact the CI is random (random
  center, random radius) and $\theta$ is fixed (though unknown). The
  instructors thus insist on the more awkward phrasing "contain," so as
  to avoid students misunderstanding. Indeed some instructors would
  contend that use of the word *in* is itself just plain incorrect.
